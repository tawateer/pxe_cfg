...
if test -b /dev/sdb
then
    echo "resize2fs /dev/sdb ;sed -i '/resize2fs/d' /etc/rc.d/rc.local " >>/etc/rc.d/rc.local
    mount -t ext4 /dev/sdb /home/
    /sbin/blkid |egrep -v "vda" |sort -u -k1 |awk '{print $2" /home/ ext4    nosuid,noatime   1 2"}' >>/etc/fstab
elif test -b /dev/xvdb
then
    echo "resize2fs /dev/xvdb ;sed -i '/resize2fs/d' /etc/rc.d/rc.local " >>/etc/rc.d/rc.local
    mount -t ext4 /dev/xvdb /home/
    /sbin/blkid |egrep -v "vda" |sort -u -k1 |awk '{print $2" /home/ ext4    nosuid,noatime   1 2"}' >>/etc/fstab
fi


mkdir -p /usr/local/bin/op/
echo 'export PATH=$PATH:/usr/local/bin/op' >>/etc/profile
